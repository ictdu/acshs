                ...
                <div class="row">
                    <div class="span3">
                        <div class="sidebar">
                            ...
                            <ul class="widget widget-menu unstyled">
                                ...
                                <li><a @if($row->subject_id == $selectedSubject->id) class="active" @endif href="{{ route('teacher.section.subject', [$section['id'] ,$row->subject_id]) }}">{{ $row->subject_code }} (Input Students Grades)</a>
                                </li>
                                @endforeach
                            </ul>
                    ...
                    <div class="span9">
                        ...
                        <div class="alert alert-success">
                          ...
                          <strong>Grades has been saved.</strong>
                        </div>
                        @endif
                        <div class="content">
                            <div class="module">
                                <div class="module-head">
                                    <h3 style="display: inline;"><i class="menu-icon icon-user"></i> Students</h3>
                                    @if($gradeBlock == false)
                                    <button type="submit" form="grade-form" class="btn btn-md btn-success btn-xs" style="float: right;">Save Grades</button>
                                    @endif
                                    <a href="{{ route('teacher.section.subject.excel', [$section['id'], $selectedSubject->id, $selectedTeacherID]) }}" class="btn btn-md btn-success btn-xs" style="float: right; margin-right: 10px;">Download Excel File</a>
                                </div>
                                <div class="module-body table">
                                  <form id="grade-form" method="POST" action="{{ route('submit.grade', [$section['id'], $selectedSubject->id, $selectedTeacherID]) }}">
                                  {{ csrf_field() }}
                                    <table id="studentTable" cellpadding="0" cellspacing="0" border="0" class="datatable-1 table table-bordered table-striped  display"
                                        ...
                                        <thead>
                                            <tr>
                                                ...
                                                <th>
                                                    {{ $section['semester'] == 1? 'FIRST SEMESTER FINAL GRADES':'SECOND SEMESTER FINAL GRADES' }}
                                                </th>
                                                <th>
                                                    REMARKS
                                                </th>
                                        ...
                                        <tbody>
                                            ...
                                            <tr>
                                                ...
                                                <td>{{ $row->name }}</td>
                                                @if($gradeBlock == false)
                                                <td><input max="100" type="number" name="grade_{{ $row->id }}" id="intLimitTextBox" class="span1" @foreach($grades as $grade) @if($grade->student_id == $row->id) value="{{ $grade->grade }}" @endif @endforeach></td>
                                                @else
                                                <td>@foreach($grades as $grade) @if($grade->student_id == $row->id) {{ $grade->grade }} @endif @endforeach</td>
                                                @endif
                                                <td>@foreach($grades as $grade) @if($grade->student_id == $row->id) {{ $grade->remark }} @endif @endforeach</td>
                                            </tr>
                                            @endforeach
                                        </tbody>
                                        <tfoot>
